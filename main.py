...
cactus_img = pygame.image.load('cactus_briques.png')
cactus_img_rect = cactus_img.get_rect()
cactus_img_rect.left = 0
feu_img = pygame.image.load('feu_briques.png')
feu_img_rect = feu_img.get_rect()
feu_img_rect.left = 0
...
class Dragon:
	...
	def update(self):
		...
		if self.dragon_img_rect.top <= cactus_img_rect.bottom:
			self.up = False
			self.down = True
		elif self.dragon_img_rect.bottom >= feu_img_rect.top:
			self.up = True
			self.down = False
		if self.up:
...
class Flames:
	...
	def __init__(self):
		self.flames = pygame.image.load('bouledefeu.png')
...
class Mario:
    ...
    def update(self):
        ...
        if self.mario_img_rect.bottom >= feu_img_rect.top:
            game_over()
            if SCORE > self.mario_score:
                self.mario_score = SCORE
        if self.up:
...
def check_level(SCORE):
    ...
    if SCORE in range(0, 10):
        ...
        feu_img_rect.top = WINDOW_HEIGHT - 50
        LEVEL = 1
    elif SCORE in range(10, 20):
        cactus_img_rect.bottom = 100
        feu_img_rect.top = WINDOW_HEIGHT - 100
        LEVEL = 2
    elif SCORE in range(20, 30):
        cactus_img_rect.bottom = 150
        feu_img_rect.top = WINDOW_HEIGHT - 150
        LEVEL = 3
    elif SCORE > 30:
        cactus_img_rect.bottom = 200
        feu_img_rect.top = WINDOW_HEIGHT - 200
        LEVEL = 4
...
def game_loop():
    while True:
        ...
        while True:
            ...
            canvas.blit(feu_img, feu_img_rect)
            mario.update()
            for f in flames_list:
